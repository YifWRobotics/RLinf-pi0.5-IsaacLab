...
2. **在 RLinf 中配置训练环境**：参考 ``RLinf/rlinf/envs/isaaclab/tasks/stack_cube.py``，将自定义脚本放到 ``RLinf/rlinf/envs/isaaclab/tasks``，并在 ``RLinf/rlinf/envs/isaaclab/__init__.py`` 中添加相关代码。
...
下方强化学习结果使用的是如下配置运行得到：

.. code:: bash

   export EMBODIED_PATH=/workspace/RLinf/examples/embodiment # 如果不使用 docker，也可设置为 export EMBODIED_PATH=$(pwd)/examples/embodiment
